...
void enqueue_list(pid32 pid, qid16	q, uint32 tickets)
{
	pid32 pid_lookup;
	pid_lookup = getfirst(q);
	if (isbadqid(q)) {
		return SYSERR;
	} else if (isempty(q)) {
		return EMPTY;
	}
	//struct procent* table_entry = &proctab[pid_lookup];
	while(queuetab[pid_lookup].qkey <= tickets)
	{
		pid_lookup = queuetab[pid_lookup].qnext;
	}
	queuetab[queuetab[pid_lookup].qnext].qprev = pid;
	queuetab[pid_lookup].qnext = pid;
	//kprintf("inserted at location %d", pid_lookup);
	return;
}